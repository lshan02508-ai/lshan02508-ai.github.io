...
import {articles} from "../app/blog-content";
import "../app/globals.css";

const Admin=lazy(()=>import("../app/admin"));
const MarkdownContent=lazy(()=>import("../app/markdown-renderer"));

function readRoute(){const hash=location.hash.slice(1);return hash.startsWith("/blog")||hash==="/admin"?hash:"/"}
...
function Article({slug}:{slug:string}){const a=articles.find(x=>x.slug===slug);if(!a)return <Home/>;return <main className="article-page"><SubNav/><article className="article-body"><a className="back" href="#/blog">← 返回文章列表</a><header><span>{a.category}</span><h1>{a.title}</h1><p>{a.lead}</p><div>{a.date} · 阅读 {a.readingTime}</div></header><Suspense fallback={<div className="article-rendering">正在排版正文…</div>}><MarkdownContent source={a.body}/></Suspense><div className="article-end">END · 这是一篇持续更新的实践笔记。</div></article></main>}
function App(){const route=useRoute();useLayoutEffect(()=>{document.documentElement.scrollTop=0;document.body.scrollTop=0;scrollTo(0,0)},[route]);if(route==="/admin")return <Suspense fallback={<main className="admin-loading">正在加载写作台…</main>}><Admin/></Suspense>;if(route==="/blog")return <Blog/>;if(route.startsWith("/blog/"))return <Article slug={route.slice(6)}/>;return <Home/>}
