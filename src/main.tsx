...
function useRoute(){const [route,setRoute]=useState(()=>location.hash.slice(1)||"/");useEffect(()=>{const update=()=>setRoute(location.hash.slice(1)||"/");addEventListener("hashchange",update);return()=>removeEventListener("hashchange",update)},[]);return route}
...
function App(){const route=useRoute();useEffect(()=>scrollTo(0,0),[route]);if(route==="/blog")return <Blog/>;if(route.startsWith("/blog/"))return <Article slug={route.slice(6)}/>;return <Home/>}
